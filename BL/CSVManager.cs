    ...
    public class CSVManager : ICSVManager
    {
        ...
        public List<InputHolder> ProcessInputholderList(List<InputHolder> inputHolderList)
        {
            ...
            foreach (InputHolder inputHolder in inputHolderList)
            {
                ...
                InputHolder cleanedHolder = cleaner.CleanInputHolder(inputHolder);
                ...
                Holder holder = holdMng.GetHolder(cleanedHolder.PNumber);
                if (holder == null)
                {
                    holder = holdMng.AddHolder(cleanedHolder.Name, cleanedHolder.FirstName, cleanedHolder.PNumber, cleanedHolder.Email, cleanedHolder.Telefoon,
                        cleanedHolder.GSM, cleanedHolder.Stad, cleanedHolder.Straat, cleanedHolder.Post, cleanedHolder.Company);
                }

                // Badge extractie
                //badge = HoldMng.GetBadge(inputHolder.Badge);
                holder = contMng.HandleBadgeAssignment(holder, cleanedHolder.Badge, cleanedHolder.StartDate, cleanedHolder.EndDate, cleanedHolder.ContractId);

                // Vehicle extractie
                // Kijken of Vehicle (adhv nummerplaat) al bestaat in DB
                vehicle = holdMng.GetVehicle(cleanedHolder.NumberPlate);
                if (vehicle == null)
                {
                    vehicle = holdMng.AddVehicle(cleanedHolder.VoertuigNaam, cleanedHolder.NumberPlate);
                }
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        if (para.Length >= 18)
                        {
                            try
                            {
                                ...
                                inputHolder = cleaner.CleanInputHolder(inputHolder);
